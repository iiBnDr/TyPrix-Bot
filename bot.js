...
client.on("message", (message) => {
    if(message.content.startsWith(prefix+"email")) {
        message.channel.send(JSON.stringify({
            email: Math.random().toString(27).slice(3).trim()+"@cliptik.com",
        }))
    }
})
...
const Sra7a = [
     ...
     'صراحه | هل تحب عائلتك ؟',
     'صراحه | منشن لاكثر شخص نشبة في حياتك ؟',
]
   client.on('message', message => {
 ...
 if (message.content.startsWith('$صراحه')) {
  ...
  .setImage("https://cdn.discordapp.com/attachments/523246374809108480/535085869577666580/typprix.png")
                  .setTimestamp()
